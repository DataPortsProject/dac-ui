...
function resize(el, binding, vnode, inserted) {
    ...
    if (vnode.componentInstance) {
      Array.from(el.children).forEach(function (child) {
        ...
        if (child.style.marginLeft !== '') {
          totalWidth += child.style.marginLeft;
        }
        if (child.style.marginRight !== '') {
          totalWidth += child.style.marginRight;
        }
      });
    } else {
    ...
    if (vnode.componentInstance) {
      Array.from(el.children).forEach(function (child) {
        ...
        if (child.style.marginTop !== '') {
          totalHeight += child.style.marginTop;
        }
        if (child.style.marginBottom !== '') {
          totalHeight += child.style.marginBottom;
        }
      });
    } else {
  ...
  if (inserted) {
    ...
    if (!gridItem) {
      console.warn(
        '[v-auto-size] Failed to retrieve grid item parent on insert'
      );
      return;
    }

    gridItem.$on('resized', () =>
      setTimeout(() => resize(el, binding, vnode, false), 0)
    );
  }
}
...
export default {
  ...
  (resizableElements = resizableElements.filter(
    item => item.vnode !== vnode
  )),
  inserted: (el, binding, vnode) =>
    setTimeout(() => resize(el, binding, vnode, true), 100),
