<template>
  <div class="app-container" :style="bgc">
    ...
    <el-row style="background:#fff;margin-bottom:30px;">
      ...
      >
        <el-table-column
          :label="this.$t('agentList.tableColumnName')"
          align="center"
          :sortable="true"
          sort-by="name"
        >
        ...
        <el-table-column
          :label="this.$t('agentList.tableColumnTag')"
          align="center"
        >
          <template slot-scope="{ row }">
            <span>{{ row.tag }}</span>
        ...
        <el-table-column
          :label="this.$t('agentList.tableColumnType')"
          align="center"
        >
          <template slot-scope="{ row }">
            <span>{{
        ...
        >
          ...
          <template slot-scope="{ row }">
            ...
            <!--<el-button v-else size="small" class="testBtn" @click="startAgent(row, true)">
    ...
    <el-dialog
      :visible.sync="startAgentDialog"
      :title="this.$t('agentList.startAgentDialogTitle')"
    >
      <el-form
        ref="initialTemplate"
        :rules="rules"
      ...
      >
        <div v-if="initialTemplate.env.length > 0">
          ...
          <div
            v-for="(item, indexTemplate) in initialTemplate.env"
            :key="indexTemplate"
          >
            ...
            <div v-if="item.type === 'number'">
              ...
              >
                <el-input-number
                  v-model.number="initialTemplate.env[indexTemplate].value"
                  :min="1"
                  style="width:80%"
                />
              </el-form-item>
            ...
            <div v-if="item.type === 'agent_type' || item.type === 'time_unit'">
              ...
              >
                <el-select
                  v-if="item.type === 'agent_type'"
                  v-model="initialTemplate.env[indexTemplate].value"
                  placeholder="Choose an option"
                  style="width:80%"
                >
                  <el-option
                    v-for="register in AgentTypes"
                    :key="register.value"
                ...
                <el-select
                  v-if="item.type === 'time_unit'"
                  v-model="initialTemplate.env[indexTemplate].value"
                  placeholder="Choose an option"
                  style="width:80%"
                >
                  <el-option
                    v-for="register in TimeUnit"
                    :key="register.value"
                    :label="register.label"
                    :value="register.value"
                  />
    ...
    >
      ...
      >
        <el-row align="center">
          ...
          <!-- Repositorio privado??? -->
          <!-- <el-form-item prop="privateRepository">
            <el-checkbox
              ref="privateRepository"
              v-model="dataImageForm.privateRepository"
              style="width:100%"
            >
              {{ this.$t("agentList.privateRepository") }}
            </el-checkbox>
          </el-form-item>
          <el-form-item
            v-if="dataImageForm.privateRepository"
            :label-width="label_widthColumn"
          ...
          <el-form-item
            v-if="dataImageForm.privateRepository"
            :label-width="label_widthColumn"
            :label="$t('agentList.addImageDialogPass')"
            prop="pass"
          >
            <el-input
              ref="pass"
              v-model="dataImageForm.pass"
              style="width:100%"
            />
          </el-form-item> -->
          ...
          >
            ...
            <el-select v-model="dataImageForm.imageSelected" style="width:100%">
              <el-option
    ...
    <!-- DIALOG TO DEVELOP / CREATE AN AGENT -->
    <el-dialog
      :title="dialogTitleCreateAgent"
      :visible.sync="dialogFormCreateAgentVisible"
      @close="handleClose_Wizard"
    >
      <el-container>
        <el-header height="100px">
          <el-steps
            :active="activeStepWizard"
        ...
        <el-main>
          <el-row v-if="activeStepWizard == 0">
            <el-col :span="24">
              <el-row :gutter="20">
                <el-col :span="8">
                  <div @click="selectDataSource = 'agent-api'">
                    <el-card
                      header="API"
                      :class="{
                ...
                <el-col :span="8">
                  <div @click="selectDataSource = 'agent-local-txt'">
                    <el-card
                      header="LOCAL TXT FILE"
                      :class="{
                ...
                <el-col :span="8">
                  <div @click="selectDataSource = 'agent-local-excel'">
                    <el-card
                      header="LOCAL EXCEL FILE"
                      :class="{
    ...
    <el-dialog
      ...
      width="20%"
      :title="this.$t('agentList.deleteImageDialogTitle')"
    >
      <div>
    ...
    <el-dialog
      ...
      width="40%"
      :title="this.$t('agentList.downloadingTemplate')"
    >
  ...
  methods: {
    ...
    async createEntities_To_ORION(id, imageType) {
      ...
      if (imageType === dataportsConstants.ON_DEMAND) {
        ...
        var entityAgentImage = buildAgentImage(
          data.Id,
          data.image,
          dataSourceID
        )
        ...
        var entityAgentContainer = buildAgentContainer(
          data.Id,
          data.image,
          dataSourceID
        )
    ...
    createAgentTemplate() {
      ...
      console.log(this.selectTypeAgent);
    ...
    obtainDataModel_Properties() {
      ...
      if (this.activeStepWizard === 3) {
        // this.formValues.orion_url = 1983
        // console.log(this.formValues)
        // console.log(this.formConfig)
