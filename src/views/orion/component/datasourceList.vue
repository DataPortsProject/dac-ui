<template>
  <div class="app-container" :style="bgc">
    ...
    <el-row style="background: #fff; margin-bottom: 30px">
      <!-- CONTAINER TABLE -->
      <el-table :key="pubSubAgentsTableKey" v-loading="listLoading" :data="filteredDatasourceByPagination" border fit
        style="width: 100%">
        ...
        <el-table-column :label="getDatamodelsTableColumn" align="center" :min-width="50">
          <template slot-scope="{ row }">
            <span><a :href="row.dataModels.value" target="_blank">{{
                            row.dataModels.value
                            }}</a></span>
          </template>
        </el-table-column>

        <el-table-column :min-width="20" :label="getActionsTableColumn" align="center"
          class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <!-- Inspect agent -->
            <el-button size="small" class="editBtn" @click="inspectDatamodel(row)" :disabled="disableButtons">
            ...
            <el-button size="small" class="editBtn" @click="viewAgentsAsociated(row)" :disabled="disableButtons">
    ...
    <pagination v-show="filteredDatasourceBySearchTextLength > 0" :total="filteredDatasourceBySearchTextLength"
      :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />

    <!-- DIALOG TO SHOW THE JSON METADATA -->
    <el-dialog :visible.sync="viewDatamodelDetailsDialog" width="80%" :title="$t('datasourceList.viewInspectDatamodel')">
    ...
    <el-dialog :visible.sync="viewAgentsDialog" width="80%" title="Agents associated">
      <el-tabs dark slider-color="green" centered color="green" v-model="mainTab" @tab-click="refreshAgentsTable++">
        <el-tab-pane label="PUBLISH-SUBSCRIBE" name="pubsubTab">
          <el-card class="box-card">
            <div class="filter-container">
              <el-input v-model="pubsubSearchText" placeholder="Search" style="width: 200px" class="filter-item"
                @keyup.enter.native="handleFilter" />
              <el-button v-waves class="filter-item" style="
                      margin-left: 10px;
                      border-color: #1890ff;
              ...
              <el-button v-waves class="filter-item" style="
                      ...
                      color: #1890ff;
                      background-color: rgb(240, 242, 245);
                    " icon="el-icon-refresh" @click="getPubSubAgents" />
            </div>
            <el-row style="background: #fff; margin-bottom: 30px">
              <!-- TABLE AGENTS PUB-SUB -->
              <el-table :key="pubSubAgentsTableKey" v-loading="pubSubAgentsLoading"
                :data="filteredPubSubAgentsByPagination" border fit style="width: 100%">
                <el-table-column label="Datasource" align="center" :sortable="true" sort-by="name" :min-width="30">
                  <template slot-scope="{ row }">
        ...
        <!-- ON DEMAND TABLE -->
        <el-tab-pane label="ON DEMAND" name="ondemandTab" :key="refreshAgentsTable">
          <el-card class="box-card">
            <div class="filter-container">
              <el-input v-model="ondemandSearchText" placeholder="Search" style="width: 200px" class="filter-item"
                @keyup.enter.native="handleFilter" />
              <el-button v-waves class="filter-item" style="
                      margin-left: 10px;
                      border-color: #1890ff;
              ...
              <el-button v-waves class="filter-item" style="
                      ...
                      color: #1890ff;
                      background-color: rgb(240, 242, 245);
                    " icon="el-icon-refresh" @click="getOnDemandAgents" />
            </div>
            <el-row style="background: #fff; margin-bottom: 30px">
              <!-- TABLE AGENTS PUB-SUB -->
              <el-table :key="onDemandAgentsTableKey" v-loading="onDemandAgentsLoading"
                :data="filteredOnDemandAgentsByPagination" border fit style="width: 100%">
                <el-table-column label="Datasource" align="center" :sortable="true" sort-by="name" :min-width="30">
                  <template slot-scope="{ row }">
                    <span>{{ row.refDataSource }}</span>
                  </template>
...
import {
  getORIONEntitiesByType,
  checkORIONAgentsAsociated,
} from "@/api/orion_API";
...
export default {
  ...
  methods: {
    ...
    async getPubSubAgents() {
      ...
      await checkORIONAgentsAsociated(this.datasourceId, "AgentContainer").then(
        (response) => {
    ...
    async getOnDemandAgents() {
      ...
      await checkORIONAgentsAsociated(this.datasourceId, "AgentImage").then(
        (response) => {
